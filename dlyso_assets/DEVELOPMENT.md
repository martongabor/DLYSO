...
QT_QPA_PLATFORM=offscreen python tools/capture_desktop.py --project /path/to/completed-public-sed-project --modality SEDplot
...
The desktop screenshots use the current shipped catalogue for setup and a completed SED workflow on the same 20 mixed YSO/non-YSO sources for results and activity. Scores, SED images and saved task states come from the actual run; no synthetic result rows or invented activity states are used. The capture tool requires an explicit completed public example project. Regenerate them after visual changes and inspect the images at normal and minimum window sizes.
