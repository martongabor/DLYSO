...
Local release-candidate checks performed on 14–24 September 2026. This records software behavior, not a scientific benchmark.
...
| Check | Outcome |
|---|---|
| Offline regression and desktop interaction suite | 67 tests passed on 2026-09-24. |
...
| Anonymous ZTF API request | Earlier check: HTTP 200, 728 measurements, no authorization header or cookies. New example check on 2026-09-24: 3C 279 returned 88 usable i-band points without authentication; eight other new-source requests timed out after retries, recorded in examples/ztf_status.csv. |
...
| Ordinary-SED workflow | All 20 mixed example sources passed download, plotting, both custom classifiers, ten ensemble models and the final join on CPU; each has 12 predictions. Labels and CSV exports were verified. |
...
| Automatic data setup | Terminal progress stays on one line and ends with a newline; redirected output stays line-oriented. Offline tests cover verified extraction, reuse, corrupt archives, unsafe archive paths and interrupted downloads. A fresh setup using the private GitHub Release downloaded, extracted and verified all 48 checkpoints. Existing CSFD maps were checked; a new CSFD network download was not repeated. |
...
Screenshots under `docs/images/` show the current 20-source mixed example catalogue and its actual completed CPU SED classification run (2026-09-24). All 12 SED classifiers were run on VizieR photometry for all 20 sources. The Results screenshot includes the new per-channel Classification column, and Project shows full valid/total row counts. Project, results, activity and minimum-size images were regenerated from current Qt widgets. `docs/screenshot-provenance.json` records the inputs and result checksum. No private catalogue is included in release documentation or archives.
