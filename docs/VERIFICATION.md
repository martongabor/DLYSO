...
| Automatic data setup | Offline tests cover verified extraction, reuse, corrupt archives, unsafe archive paths and interrupted downloads. All 48 existing models and installed CSFD maps were checked by the real setup command. |
| In-app documentation | Relative GitHub installation link loads Markdown content; Back returns to the user guide. |
| Git installation | pip installation from a temporary local Git repository passed; installed CLI modules, GUI and bundled resources loaded outside the checkout. Remote GitHub installation awaits repository access and publication; dependencies were reused from the tested environment. |
